...
public class PhotonClientImpl: MonoBehaviourPunCallbacks, MultiPlayClient, IOnEventCallback
{
    ...
    private const byte Code_PlayerMove = 0x01;
    private const byte Code_ObjectMove = 0x02;

    public void init(MultiPlayClientCallback clientCallback)
    {
        this.clientCallback = clientCallback;

        PhotonPeer.RegisterType(typeof(PlayerInfo), (byte)'A', PhotonSerialize.PlayerSerializeMethod, PhotonSerialize.PlayerDeserializeMethod);
    ...
    public List<PlayerInfo> getPlayerList()
    {
        return PhotonNetwork.PlayerList.Select(player =>
        {
            ...
            PlayerInfo moveData;
            if (property.ContainsKey(KEY_playerMoveData))
            {
                moveData = (PlayerInfo)property[KEY_playerMoveData];
            } else
            {
                moveData = new PlayerInfo(userID: getUserID());
            }
    ...
    public void playerMove(PlayerInfo playerInfo)
    {
        ...
        customProperties[KEY_playerMoveData] = playerInfo;


        Debug.Log("playerMove " + customProperties);
        PhotonNetwork.LocalPlayer.SetCustomProperties(customProperties);

        object[] contents = new object[] { playerInfo };
        sendEvent(eventCode: Code_PlayerMove, contents: contents);
    }


    public void objectMove(string triggerByID, RoomItem item)
    {
        sendEvent(eventCode: Code_ObjectMove, contents: new object[] { item });
    ...
    public override void OnRoomPropertiesUpdate(Hashtable propertiesThatChanged)
    {
        ...
        Debug.Log("OnRoomPropertiesUpdate : UserId");
    ...
    private void sendEvent(byte eventCode, object[] contents)
    {
        RaiseEventOptions raiseEventOptions = new RaiseEventOptions { Receivers = ReceiverGroup.Others };
        PhotonNetwork.RaiseEvent(eventCode, contents, raiseEventOptions, SendOptions.SendReliable);
    }

    public void OnEvent(EventData photonEvent)
    {
        var eventCode = photonEvent.Code;
        var contentObj = (object[]) photonEvent.CustomData;
        ...
        switch (eventCode)
        {
            case Code_PlayerMove:
                PlayerInfo playerData = contentObj[0] as PlayerInfo;
                if (playerData != null && playerData.userID != null && playerData.userID != "")
                {
                    clientCallback.onOtherPlayerMove(
                ...
                break;
            case Code_ObjectMove:
                RoomItem item = contentObj[0] as RoomItem;
    ...
    class PhotonSerialize
    {
        ...
        public static byte[] PlayerSerializeMethod(object customObject)
        {
            ...
            {
                var json = JsonUtility.ToJson((PlayerInfo)customObject);
                var jsonByte = Encoding.ASCII.GetBytes(json);
                return jsonByte;
            }
        }
